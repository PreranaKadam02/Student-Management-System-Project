...
username = urllib.parse.quote_plus('kpreranaao2')
password = urllib.parse.quote_plus('5xvW4PRWNY9fKGKx')
connection_string = f"mongodb+srv://{username}:{password}@cluster0.1os4hvs.mongodb.net/?retryWrites=true&w=majority"
...
def add_STUDENTS(root,db): 
    def add_query():
        ...
        if(len(prn)==0):
            messagebox.showwarning.showwarning("WARNING", "All fields are compulsory(Except: Mobile number)")
            return
        if(len(name)==0):
